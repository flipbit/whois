...
namespace Whois.Parsing.Whois.Nic.Tech.Tech
{
    public class TechParsingTests : ParsingTests
    {
        ...
        {
            var sample = SampleReader.Read("whois.nic.tech", "tech", "found", "found.txt");
            var response = parser.Parse("whois.nic.tech", sample);
            ...
            Assert.Equal(new DateTime(2015, 08, 04, 09, 00, 43, 000, DateTimeKind.Utc), response.Updated);
            Assert.Equal(new DateTime(2015, 07, 29, 00, 00, 00, 000, DateTimeKind.Utc), response.Registered);
            Assert.Equal(new DateTime(2017, 07, 29, 23, 59, 59, 000, DateTimeKind.Utc), response.Expiration);
