...
public class ReParsingTests : ParsingTests
{
    ...
    public void Test_found()
    {
        ...
        Assert.Equal("                  Nic re", response.Registrant.Name);
        ...
        Assert.Equal("                  Nic re", response.AdminContact.Name);
        ...
        Assert.Equal("                  CTC101926-FRNIC", response.TechnicalContact.RegistryId);
    ...
    {
        var sample = SampleReader.Read("whois.nic.re", "re", "throttled", "throttled.txt");
        var response = parser.Parse("whois.nic.re", sample);
    ...
    {
        var sample = SampleReader.Read("whois.nic.re", "re", "not-found", "not_found.txt");
        var response = parser.Parse("whois.nic.re", sample);
    ...
    {
        var sample = SampleReader.Read("whois.nic.re", "re", "found", "found_status_registered.txt");
        var response = parser.Parse("whois.nic.re", sample);
        ...
        Assert.Equal("nic.re", response.DomainName.ToString());
        ...
        // Registrant Details
        Assert.Equal("A1967-FRNIC", response.Registrant.RegistryId);
        Assert.Equal("AFNIC", response.Registrant.Name);
        ...
        Assert.Equal("78181 Saint Quentin en Yvelines", response.Registrant.Address[3]);
        Assert.Equal("FR", response.Registrant.Address[4]);
