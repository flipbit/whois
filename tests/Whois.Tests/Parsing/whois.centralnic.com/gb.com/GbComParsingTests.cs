...
namespace Whois.Parsing.Whois.Centralnic.Com.GbCom
{
    public class GbComParsingTests : ParsingTests
    {
        ...
        {
            var sample = SampleReader.Read("whois.centralnic.com", "gb.com", "not-found", "not_found.txt");
        ...
        {
            var sample = SampleReader.Read("whois.centralnic.com", "gb.com", "found", "found.txt");
            var response = parser.Parse("whois.centralnic.com", sample);
            ...
            Assert.Equal("hotel.gb.com", response.DomainName.ToString());
            Assert.Equal("CNIC-DO403461", response.RegistryDomainId);

            // Registrar Details
            Assert.Equal("Wind Internethaus GMBH", response.Registrar.Name);
            Assert.Equal("+49.77214070740", response.Registrar.AbuseTelephoneNumber);
            Assert.Equal("www.windinternethaus.de", response.Registrar.Url);

            Assert.Equal(new DateTime(2014, 2, 12, 9, 45, 17, DateTimeKind.Utc), response.Updated);
            Assert.Equal(new DateTime(2006, 4, 23, 6, 26, 11, DateTimeKind.Utc), response.Registered);
            Assert.Equal(new DateTime(2016, 4, 23, 23, 59, 59, DateTimeKind.Utc), response.Expiration);

             // Registrant Details
            Assert.Equal("H1049605", response.Registrant.RegistryId);
            Assert.Equal("Robert Ragge, Hotel Reservation Service Robert Ragge GmbH", response.Registrant.Name);
            ...
            Assert.Equal("H393781", response.AdminContact.RegistryId);
            ...
            Assert.Equal("NRW", response.AdminContact.Address[2]);
            ...
            Assert.Equal("H1103064", response.BillingContact.RegistryId);
            Assert.Equal("Uwe Watzek, Wind Internethaus GMBH", response.BillingContact.Name);

             // BillingContact Address
            Assert.Equal(5, response.BillingContact.Address.Count);
            Assert.Equal("Am Krebsgraben 15", response.BillingContact.Address[0]);
            Assert.Equal("Villingen-Schwenningen", response.BillingContact.Address[1]);
            Assert.Equal("Baden-Wuerttemberg", response.BillingContact.Address[2]);
            Assert.Equal("78048", response.BillingContact.Address[3]);
            Assert.Equal("DE", response.BillingContact.Address[4]);

            Assert.Equal("+49.77214070740", response.BillingContact.TelephoneNumber);
            Assert.Equal("+49.77214070741", response.BillingContact.FaxNumber);
            Assert.Equal("info@windinternethaus.de", response.BillingContact.Email);


             // TechnicalContact Details
            Assert.Equal("H1103064", response.TechnicalContact.RegistryId);
            Assert.Equal("Uwe Watzek, Wind Internethaus GMBH", response.TechnicalContact.Name);
