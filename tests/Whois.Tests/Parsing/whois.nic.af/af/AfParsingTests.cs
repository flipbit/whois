...
public class AfParsingTests : ParsingTests
{
    ...
    {
        var sample = SampleReader.Read("whois.nic.af", "af", "not-found", "not_found.txt");
        ...
        Assert.Equal("whois.nic.af/af/not-found/01", response.TemplateName);

        Assert.Equal("u34jedzcq.af", response.DomainName.ToString());
    ...
    {
        var sample = SampleReader.Read("whois.nic.af", "af", "found", "found.txt");
        var response = parser.Parse("whois.nic.af", sample);
        ...
        Assert.Equal("google.af", response.DomainName.ToString());
        Assert.Equal("345679-CoCCA", response.RegistryDomainId);

        // Registrar Details
        Assert.Equal("MarkMonitor", response.Registrar.Name);
        Assert.Equal("http://www.markmonitor.com", response.Registrar.Url);
        Assert.Equal("whois.nic.af", response.Registrar.WhoisServer.Value);
        ...
        Assert.Equal(new DateTime(2014, 01, 16, 06, 50, 48, 226, DateTimeKind.Utc), response.Updated);
        Assert.Equal(new DateTime(2009, 10, 05, 03, 51, 17, 979, DateTimeKind.Utc), response.Registered);
        Assert.Equal(new DateTime(2014, 05, 05, 03, 51, 17, 974, DateTimeKind.Utc), response.Expiration);

        // Registrant Details
        Assert.Equal("969675-CoCCA", response.Registrant.RegistryId);
        ...
        Assert.Equal("969675-CoCCA", response.AdminContact.RegistryId);
        ...
        Assert.Equal("969676-CoCCA", response.BillingContact.RegistryId);
        ...
        Assert.Equal("969675-CoCCA", response.TechnicalContact.RegistryId);
