...
namespace Whois.Parsing.Whois.Nic.Re.Re
{
    ...
    public class ReParsingTests : ParsingTests
    {
        ...
        public void Test_found()
        {
            ...
            Assert.AreEqual(new DateTime(2009, 03, 12, 00, 00, 00, 000, DateTimeKind.Utc), response.Updated);
            Assert.AreEqual(new DateTime(1995, 01, 01, 00, 00, 00, 000, DateTimeKind.Utc), response.Registered);

             // Registrant Details
            Assert.AreEqual("A1967-FRNIC", response.Registrant.RegistryId);
            Assert.AreEqual("AFNIC", response.Registrant.Name);
            ...
            Assert.AreEqual("78181 Saint Quentin en Yvelines", response.Registrant.Address[3]);
