...
namespace Whois.Parsing.Whois.Nic.Pm.Pm
{
    ...
    public class PmParsingTests : ParsingTests
    {
        ...
        public void Test_found()
        {
            ...
            Assert.AreEqual("nic.pm", response.DomainName.ToString());

            // Registrar Details
            Assert.AreEqual("AFNIC registry", response.Registrar.Name);

            Assert.AreEqual(new DateTime(2004, 09, 17, 00, 00, 00, 000, DateTimeKind.Utc), response.Updated);
            Assert.AreEqual(new DateTime(1995, 01, 01, 00, 00, 00, 000, DateTimeKind.Utc), response.Registered);

             // Registrant Details
            Assert.AreEqual("APEM2-FRNIC", response.Registrant.RegistryId);
            Assert.AreEqual("Afnic (Saint-Pierre et Miquelon - CTOM)", response.Registrant.Name);
