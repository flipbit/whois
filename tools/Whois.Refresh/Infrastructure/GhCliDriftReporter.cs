...
#pragma warning restore CA1031
    ...
    private static async Task CommitAndPushAsync(string branch, string repoRoot, CancellationToken cancellationToken)
    {
        ...
        await RunGitAsync("add tools/Whois.Refresh/refresh-results.json", repoRoot, cancellationToken).ConfigureAwait(false);
        await RunGitAsync("add tools/Whois.Refresh/drift-report.json tools/Whois.Refresh/drift-report.md", repoRoot, cancellationToken).ConfigureAwait(false);
